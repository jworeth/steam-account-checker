...
namespace steam_checker
{
    public static class Logging
    {
        ...
        /// 
        /// </summary>
        public delegate void SetTextCallback(RichTextBox box, string text);
        ...
        private static string LogTime { get { return DateTime.Now.ToLongTimeString().Replace(":", ".").Replace(" PM", ""); } }
        ...
        private static void _WriteQueue()
        {
            ...
            while (true)
            {
                try
                {
                    using (
                        TextWriter tw =
                            new StreamWriter(string.Format("{0}\\Logs\\{1}{2} Log.txt", ApplicationPath, LogDate, LogTime), true))
                    {
        ...
        public static void Logrtb(RichTextBox box, string text)
        {
            box.Text += TimeStamp + text + "\n";
        }
        ...
        {
            string s = string.Format("{0}{1} - {4} - From: {2}\n{3}", TimeStamp, ex.Message, ex.Source, ex.StackTrace, ex.InnerException);
            LogQueue.Enqueue(s);
        }
    }
}